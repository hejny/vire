...
export class VectorSet {
    ...
    equals(vectorSet2: VectorSet):boolean{
        return(
            this.isThisSubset(vectorSet2) &&
            vectorSet2.isThisSubset(this)
        )
    }

    addUnique(...points: Vector2[]): this {//todo add fluent API to other methods
    ...
    substract(vectorSetNegative: VectorSet):VectorSet{
        ...
        for(const point of this.points){
            if(!vectorSetNegative.points.some(pointNegative=>pointNegative.equals(point))){
                vectorSet.add(point);
            }
        }
        return vectorSet;
        ...
        function iteration(separateIslands: VectorSet[]): VectorSet[] {
            for (const vectorSet1 of separateIslands) {
                ...
                if(vectorSetsOld.length>1){
                    separateIslands = separateIslands.filter((island)=>vectorSetsOld.indexOf(island)===-1);
    ...
    boundingPointsComposite(): [Vector2, Vector2, Vector2, Vector2] {//todo DRY
        const xs = this.points.map((point) => point.x);
        const ys = this.points.map((point) => point.y);
